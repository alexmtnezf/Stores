    ...
    def test_find_store(self):
        with self.app_context():
            with self.client() as cl:
                ...
                resp = cl.get('/store/test', headers={'Authorization': 'JWT ' + jwt_token})
                self.assertEqual(200, resp.status_code)
                self.assertEqual({'name': 'test', 'items': []}, json.loads(resp.data))
